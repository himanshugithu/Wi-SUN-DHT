...
void _join_state_custom_callback(sl_wisun_evt_t *evt)
{
  ...
  if (join_state != previous_join_state)
  {
    ...
    // Print stage line for each join state (numeric, since only DISCONNECTED and OPERATIONAL are defined)
    if (join_state > SL_WISUN_JOIN_STATE_DISCONNECTED && join_state <= SL_WISUN_JOIN_STATE_OPERATIONAL)
    {
      printf("node on stage %d\n", join_state);
    }
    if ((join_state > SL_WISUN_JOIN_STATE_DISCONNECTED) && (join_state <= SL_WISUN_JOIN_STATE_OPERATIONAL))
    {
    ...
    if (join_state == SL_WISUN_JOIN_STATE_OPERATIONAL)
    {
      // Turn on LED when connected to Border Router
...
#define CONNECTED_JSON_FORMAT_STR            \
  "%s" START_JSON DEVICE_PARENT_RUNNING_JSON \
  "\"connected\":   \"%s\",\n"               \
  "\"disconnected\":\"%s\",\n"               \
  "\"connections\": \"%d\",\n"               \
  "\"availability\":\"%6.2f\",\n"            \
