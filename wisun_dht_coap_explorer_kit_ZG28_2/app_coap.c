...
#define COAP_MAX_RESPONSE_LEN 64
static char coap_response[COAP_MAX_RESPONSE_LEN];

// DHT11 /temp CoAP callback

sl_wisun_coap_packet_t *coap_callback_temp(const sl_wisun_coap_packet_t *const req_packet)
{
  printf("Received CoAP request: /temp\n");
  DHT11_Data_t dht_data;
  DHT11_Init();
  if (DHT11_Read(&dht_data))
  {
    snprintf(coap_response, COAP_MAX_RESPONSE_LEN, "Temperature : %d.%d C", dht_data.temperature_int, dht_data.temperature_dec);
    printf( "Temperature : %d.%d C", dht_data.temperature_int, dht_data.temperature_dec);
  }
  else
  {
    snprintf(coap_response, COAP_MAX_RESPONSE_LEN, "DHT read error");
  }
  return app_coap_reply(coap_response, req_packet);
}

// DHT11 /humi CoAP callback

sl_wisun_coap_packet_t *coap_callback_humi(const sl_wisun_coap_packet_t *const req_packet)
{
  printf("Received CoAP request: /humi\n");
  DHT11_Data_t dht_data;
  DHT11_Init();
  if (DHT11_Read(&dht_data))
  {
    snprintf(coap_response, COAP_MAX_RESPONSE_LEN, "Humidity : %d.%d %%", dht_data.humidity_int, dht_data.humidity_dec);
    printf("Humidity : %d.%d %%", dht_data.humidity_int, dht_data.humidity_dec);
  }
  else
  {
    snprintf(coap_response, COAP_MAX_RESPONSE_LEN, "DHT read error");
...
// DHT11 /data CoAP callback

sl_wisun_coap_packet_t *coap_callback_dht_data(const sl_wisun_coap_packet_t *const req_packet)
{
  printf("Received CoAP request: /data\n");
  DHT11_Data_t dht_data;
  DHT11_Init();
  if (DHT11_Read(&dht_data))
  {
...
uint8_t app_coap_resources_init()
{
  ...
  // DHT11 /temp endpoint
  coap_resource.data.uri_path = "/temp";
  coap_resource.data.resource_type = "text";
  coap_resource.data.interface = "sensor";
  coap_resource.auto_response = coap_callback_temp;
  coap_resource.discoverable = true;
  assert(sl_wisun_coap_rhnd_resource_add(&coap_resource) == SL_STATUS_OK);
  count++;

  // DHT11 /humi endpoint
  coap_resource.data.uri_path = "/humi";
  coap_resource.data.resource_type = "text";
  coap_resource.data.interface = "sensor";
  coap_resource.auto_response = coap_callback_humi;
  coap_resource.discoverable = true;
  assert(sl_wisun_coap_rhnd_resource_add(&coap_resource) == SL_STATUS_OK);
  count++;

  // DHT11 /data endpoint
  coap_resource.data.uri_path = "/dht_data";
  coap_resource.data.resource_type = "json";
  coap_resource.data.interface = "sensor";
  coap_resource.auto_response = coap_callback_dht_data;
  coap_resource.discoverable = true;
  assert(sl_wisun_coap_rhnd_resource_add(&coap_resource) == SL_STATUS_OK);
  count++;
