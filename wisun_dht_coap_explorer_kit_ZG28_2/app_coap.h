...
#include "sl_wisun_coap.h" // for sl_wisun_coap_packet_t
#include <stdbool.h>

// DHT endpoint callback prototypes
...
// Prototype for app_coap_reply
sl_wisun_coap_packet_t *app_coap_reply(char *response_string, const sl_wisun_coap_packet_t *const req_packet);
...
// For DHT endpoint and CoAP response buffer
#define COAP_MAX_RESPONSE_LEN 64

// Prototype for DHT endpoint callback
struct sl_wisun_coap_packet; // Forward declaration if needed
sl_wisun_coap_packet_t *coap_callback_dht_data(const sl_wisun_coap_packet_t *const req_packet);
